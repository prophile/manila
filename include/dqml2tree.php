...
class dqml2tree
{
    ...
    function _makeChildsTree(&$_tree, $parent, $grand_parent, &$inzones)
    {
        foreach ($_tree as $branch => $sub_tree) {
            if ($branch == '_SQL' && count($_tree) < 4) {
                ...
                if ($pos !== false) {
                    ...
                    if (isset($inzones[$pos])) {
                        ...
                        $this->_makeChildsTree($new_tree, '', '', $inzones[$pos]);
                        $_tree += $new_tree;
                    }
            ...
            elseif ($branch != '_SQL' && $branch != '_a' && $branch != '_z') {
                $this->_makeChildsTree($_tree[$branch], $branch, $parent, $inzones);
            }
        }
    }
    ...
    function _reduceTree(&$_tree)
    {
        while (list($branch, $sub_tree) = each($_tree)) {
            if ($branch != '_SQL' && $branch != '_a' && $branch != '_z') {
                ...
                if (isset($_tree['_a']) && isset($_tree['_z']) && $_tree['_a'] == $sub_tree['_a'] && $_tree['_z'] == $sub_tree['_z'] && count($_tree) < 5) $_tree = $sub_tree;
            }
        }
    }
    ...
    function make()
    {
        ...
        $this->_makeChildsTree($this->_tree['SQL'], 'SQL', '', $this->_zonesP_tree);
